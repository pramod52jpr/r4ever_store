...
class AdminTermsConditions extends StatefulWidget {
  const AdminTermsConditions({super.key});

  @override
  State<AdminTermsConditions> createState() => _AdminTermsConditionsState();
}

class _AdminTermsConditionsState extends State<AdminTermsConditions> {
  TextEditingController termsCondition = TextEditingController();
  GlobalData? data;
  ...
  void fetchGlobalData({bool firstLoad = true}) async {
    try {
      ...
      if (resp.status) {
        data = resp.data;
        termsCondition.text = resp.data!.termsConditions;
  ...
  void changeTermsAndConditions() async{
    try{
      setState(() {
        updateLoading = ApiStatus.loading;
      });
      var res = await ApiServices().postRequest("plan/update_terms_conditions", data: {
        "termsConditions": termsCondition.text,
      });
